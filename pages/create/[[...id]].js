...
const ProductsManager = () => {
    ...
    const handleUploadInput = e => {
        ...
        files.forEach(file => {
            if(file.size > 6024 * 6024)
            return err = 'The largest image size is 6mb'

            if(file.type !== 'image/jpeg' && file.type !== 'image/png')
            return err = 'Image format is incorrect.'

            num += 1;
            if(num <= 8) newImages.push(file)
            return newImages;
        })
        ...
        if(imgCount + newImages.length > 8)
        return dispatch({type: 'NOTIFY', payload: {error: 'Select up to 8 images.'}})
    ...
    return(
        <div className="products_manager">
            ...
            <form className="row fill-product" onSubmit={handleSubmit}>
